...
std::vector<short> generateSineWave(float frequency, float duration, int sampleRate) {
    int sampleCount = static_cast<int>(duration * sampleRate);
    std::vector<short> samples(sampleCount);

    float deltaT = 1.0f / sampleRate;
    for (int i = 0; i < sampleCount; i++) {
        samples[i] = static_cast<short>(32767.0f * sin(2.0f * PI * frequency * i * deltaT));
    }
...
int main() {
    ...
    alBufferData(buffer, AL_FORMAT_MONO16, waveData.data(), waveData.size() * sizeof(short), sampleRate);
